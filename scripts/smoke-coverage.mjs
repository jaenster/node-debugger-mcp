// Isolate the coverage tool — no patching, no exceptions, just record + report.
...
async function main() {
  ...
  console.log("=== launch multi-bp.js stopOnEntry:true ===");
  console.log(text(await call("debug_launch", { script: fixturePath, stopOnEntry: true, followChildren: "off" })));

  console.log("\n=== coverage BEFORE resume (should capture as script runs to completion) ===");
  // Start coverage by calling the tool — it captures durationMs of execution.
  // Need to resume first so the script can run.
  const covPromise = call("debug_coverage", { durationMs: 1500, urlFilter: "multi-bp" });
  await wait(50);
  await call("debug_continue", { waitForPause: false });
  const covRes = await covPromise;
  console.log(text(covRes));
